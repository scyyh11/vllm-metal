...
"""Perf benchmark for the materialized-MLA prefill fast path
(``VLLM_METAL_MLA_MATERIALIZED_PREFILL``). GLM-4.7-Flash dims, on a varlen batch
of prefill requests:
...
Both use only public MLX ops (no custom kernel); both are bitwise-equal (the
absorption identity). Reports the materialized speedup.
...
def main() -> None:
    ...
    print(f"H={H}, qk={QK}, v={VD}, fp16, causal, p50\n")
    ...
    for lens in [
        ...
        def absorbed(lens=lens, cu=cu, qn=qn, qp=qp, kvn=kvn, kpe=kpe):
            outs = []
        ...
        def materialized(lens=lens, cu=cu, qn=qn, qp=qp, kvn=kvn, kpe=kpe, total=total):
            k_nope = eq(kvn, transpose=False)  # [H, total, qk_nope]
            values = uo(kvn)  # [H, total, v]
            kp = mx.broadcast_to(kpe[None], (H, total, ROPE))
            keys = mx.concatenate([k_nope, kp], axis=-1)  # [H, total, qk]
            queries = mx.concatenate([qn, qp], axis=-1).transpose(
                1, 0, 2
            )  # [H,total,qk]
            outs = []
