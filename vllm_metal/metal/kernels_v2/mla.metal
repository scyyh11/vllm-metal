...
// ========================================== Per-request MMA paged main
//
// Full main kernel for the per-request MMA paged decode path.
//
// Per TG: one (seq, partition, head_group of HEADS_PER_TG heads).
// Per simdgroup tile inside TG: one [8-head × 8-K-token] MMA sub-tile.
// WM=4 × WN=8 = 32 simdgroups = 1024 threads (M5 Max TG ceiling).
//
// I/O contract identical to paged_mla_attention_decode_2pass_1 (so the
// existing paged_mla_attention_reduce kernel consumes the partials):
//   exp_sums   [num_seqs, num_heads, num_partitions]              fp32
//   max_logits [num_seqs, num_heads, num_partitions]              fp32
//   tmp_out    [num_seqs, num_heads, num_partitions, KV_LORA_RANK] T (normalized partial)
//
// Q input is combined [num_seqs, num_heads, KV_LORA_RANK + QK_ROPE_HEAD_DIM]
// (caller responsibility — wrapper concats q_nope and q_pe before dispatch).
template <typename T, int KV_LORA_RANK, int QK_ROPE_HEAD_DIM, int BLOCK_SIZE,
          int PARTITION_SIZE, int HEADS_PER_TG, int WM, int WN>
[[kernel]] void paged_mla_attention_decode_pr_mma_main(
    device float *exp_sums       [[buffer(0)]],
    device float *max_logits     [[buffer(1)]],
    device T *tmp_out            [[buffer(2)]],
    device const T *q_combined   [[buffer(3)]],
    device const T *latent_cache [[buffer(4)]],
    device const uint32_t *block_tables [[buffer(5)]],
    device const uint32_t *context_lens [[buffer(6)]],
    ...
    uint simd_gid [[simdgroup_index_in_threadgroup]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  constexpr int D_COMBINED = KV_LORA_RANK + QK_ROPE_HEAD_DIM;
  constexpr int FRAG = 8;
  constexpr int BM = WM * FRAG;
  static_assert(HEADS_PER_TG == BM, "HEADS_PER_TG must equal WM * FRAG");
  constexpr int N_PER_SG_QK = PARTITION_SIZE / (WN * FRAG);  // 2
  static_assert(PARTITION_SIZE % (WN * FRAG) == 0,
                "PARTITION_SIZE must be divisible by WN*FRAG");
  static_assert(KV_LORA_RANK % (WN * FRAG) == 0,
                "WN simdgroups partition KV_LORA_RANK; must be divisible by WN*FRAG");
  static_assert(D_COMBINED % FRAG == 0, "D_COMBINED must be divisible by FRAG");
  static_assert(PARTITION_SIZE % BLOCK_SIZE == 0,
                "PARTITION_SIZE must be divisible by BLOCK_SIZE");
  static_assert(WM * WN == BM,
                "Softmax assigns one head row per simdgroup; need WM*WN == BM");
  static_assert(PARTITION_SIZE % 32 == 0,
                "PARTITION_SIZE must be divisible by 32 for lane-stride softmax");

  constexpr int D_TILES = D_COMBINED / FRAG;            // 72
  constexpr int K_TILES = PARTITION_SIZE / FRAG;        // 16
  constexpr int V_TILES_PER_SG = KV_LORA_RANK / (WN * FRAG);  // 8
  constexpr int LD_S = PARTITION_SIZE + FRAG;           // 136 (padded)
  constexpr int COLS_PER_LANE = PARTITION_SIZE / 32;    // 4

  threadgroup float S_smem[BM * LD_S];                  // 32 × 136 × 4 = 17 KB
  threadgroup float row_max_smem[BM];
  threadgroup float row_sum_smem[BM];

  const int seq_idx = (int)tg.x;
  const int partition_idx = (int)tg.y;
  const int head_group_idx = (int)tg.z;
  const int num_partitions = (int)tpg.y;
  const int sg_m = (int)simd_gid / WN;
  const int sg_n = (int)simd_gid % WN;
  const int tid = (int)simd_gid * 32 + (int)simd_lid;
  const int num_heads = num_heads_total;
  const int q_head_offset = head_group_idx * BM;

  if (q_head_offset >= num_heads) {
    return;
  }
  ...
  // Empty partition: emit -INF / 0 sentinels for the reduce.
  if (token_start >= ctx_len) {
    if (tid < BM) {
      const int head = q_head_offset + tid;
      if (head < num_heads) {
        max_logits[(seq_idx * num_heads + head) * num_partitions + partition_idx] =
            -INFINITY;
        exp_sums[(seq_idx * num_heads + head) * num_partitions + partition_idx] =
            0.0f;
      }
    }
    return;
  }
  const int valid_cols = token_end - token_start;

  // BaseMMAFrag lane → (row, col) layout. Each lane owns 2 elements at
  // (fm, fn) and (fm, fn+1) in any 8x8 simdgroup_matrix tile.
  const short qid = simd_lid / 4;
  const short fm = (qid & 4) + ((simd_lid / 2) % 4);
  const short fn = (qid & 2) * 2 + (simd_lid % 2) * 2;

  // Per-seq Q base (combined Q in memory).
  const device T *q_seq_base = q_combined +
      (uint64_t)(seq_idx * num_heads + q_head_offset) * D_COMBINED;
  ...
  auto k_base_for = [&](int abs_k) -> const device T * {
    ...
    const int pbi =
        pbi_raw < max_num_blocks_per_seq ? pbi_raw : max_num_blocks_per_seq - 1;
    const int offset = abs_k % BLOCK_SIZE;
    const uint32_t pb = block_table_row[pbi];
    return latent_cache + (uint64_t)pb * BLOCK_SIZE * D_COMBINED +
           (uint64_t)offset * D_COMBINED;
  };

  // === Phase 1: QK MMA. Each simdgroup produces N_PER_SG_QK 8x8 S tiles. ===
  // S_tile = scale * (Q[heads] · K[tokens]^T) — scale folded post-MMA.
  for (int n_sub = 0; n_sub < N_PER_SG_QK; n_sub++) {
    simdgroup_matrix<float, FRAG, FRAG> S_tile(0.0f);
    const int n_local_tok_start = (sg_n * N_PER_SG_QK + n_sub) * FRAG;
    const int n_abs_tok_start = token_start + n_local_tok_start;
    const device T *k_base = k_base_for(n_abs_tok_start);
    const device T *q_ptr = q_seq_base + (uint64_t)(sg_m * FRAG) * D_COMBINED;

    for (int d = 0; d < D_COMBINED; d += FRAG) {
      simdgroup_matrix<T, FRAG, FRAG> Q_frag;
      simdgroup_matrix<T, FRAG, FRAG> K_frag;
      simdgroup_load(Q_frag, q_ptr + d, D_COMBINED);
      // K is [tokens × D] in memory; transpose for B operand (D × tokens).
      simdgroup_load(K_frag, k_base + d, D_COMBINED,
                     ulong2(0, 0), /*transpose=*/true);
      simdgroup_multiply_accumulate(S_tile, Q_frag, K_frag, S_tile);
    }
    // Fold the attention scale into S after the MMA accumulator finishes.
    S_tile.thread_elements()[0] = float(S_tile.thread_elements()[0]) * scale;
    S_tile.thread_elements()[1] = float(S_tile.thread_elements()[1]) * scale;
    simdgroup_store(S_tile,
        &S_smem[(uint64_t)(sg_m * FRAG) * LD_S + n_local_tok_start], LD_S);
  }
  threadgroup_barrier(mem_flags::mem_threadgroup);

  // === Phase 2: per-head softmax. One head row per simdgroup. ===
  // 32 simdgroups × 32 lanes = 1024 threads; 32 heads × 128 K tokens = 4096
  // elements; 32 head rows / 32 simdgroups → 1 row per simdgroup; 128 K
  // tokens / 32 lanes → 4 cols per lane (COLS_PER_LANE).
  {
    const int head_row = (int)simd_gid;
    float local_x[COLS_PER_LANE];
    float local_max = -INFINITY;
    for (int c = 0; c < COLS_PER_LANE; c++) {
      const int col = c * 32 + (int)simd_lid;
      const float x_raw = S_smem[head_row * LD_S + col];
      const float x = (col < valid_cols) ? x_raw : -INFINITY;
      local_x[c] = x;
    ...
    for (int c = 0; c < COLS_PER_LANE; c++) {
      const int col = c * 32 + (int)simd_lid;
      const float exp_x =
          (col < valid_cols) ? fast::exp(local_x[c] - row_max) : 0.0f;
      S_smem[head_row * LD_S + col] = exp_x;
      local_sum += exp_x;
    }
    const float row_sum = simd_sum(local_sum);
    if (simd_lid == 0) {
      row_max_smem[head_row] = row_max;
      row_sum_smem[head_row] = row_sum;
    }
  }
  threadgroup_barrier(mem_flags::mem_threadgroup);

  // === Phase 3: SV MMA. V is the first KV_LORA_RANK cols of latent_cache. ===
  // Each simdgroup owns 8 heads × 64 V cols (V_TILES_PER_SG=8 tiles of 8x8).
  // V_accum starts at 0 (no online softmax across partitions — each
  // partition is independent in the 2pass + reduce structure).
...
#pragma unroll
  ...
  // Pre-compute K-token base pointers per K-tile (each tile = FRAG=8 tokens
  // entirely within one block since PARTITION_SIZE/FRAG=16 chunks fit 16
  // chunks across 8 blocks at BLOCK_SIZE=16).
  const device T *v_k_base[K_TILES];
#pragma unroll
  for (int kt = 0; kt < K_TILES; kt++) {
    v_k_base[kt] = k_base_for(token_start + kt * FRAG);
  }

  // V_accum[vt] = sum_kt weights[heads, kt_tokens] · V[kt_tokens, v_cols]
  // S_smem (after softmax) holds exp(s - max) — not yet divided by sum.
  for (int vt = 0; vt < V_TILES_PER_SG; vt++) {
    const int v_col_start = (sg_n * V_TILES_PER_SG + vt) * FRAG;
    for (int kt = 0; kt < K_TILES; kt++) {
      const int k_inner = kt * FRAG;
      simdgroup_matrix<float, FRAG, FRAG> S_frag;
      simdgroup_matrix<T, FRAG, FRAG> V_frag;
      simdgroup_load(S_frag, &S_smem[(sg_m * FRAG) * LD_S + k_inner], LD_S);
      // V row of FRAG K tokens × FRAG V cols; V is the first KV_LORA_RANK
      // cols of each latent row, so the stride is D_COMBINED.
      simdgroup_load(V_frag, v_k_base[kt] + v_col_start, D_COMBINED);
      simdgroup_multiply_accumulate(V_accum[vt], S_frag, V_frag, V_accum[vt]);
    }
  }

  // === Phase 4: normalize partial by inv_sum, write outputs ===
  // Per-head writes happen at lane granularity via the BaseMMAFrag layout.
  const int head_in_tg = sg_m * FRAG + fm;
  const int head_global = q_head_offset + head_in_tg;
  if (head_global < num_heads) {
    const float row_sum = row_sum_smem[head_in_tg];
    const float inv_sum = (row_sum > 0.0f) ? (1.0f / row_sum) : 0.0f;
    device T *out_row =
        tmp_out + ((uint64_t)(seq_idx * num_heads + head_global) * num_partitions +
                   partition_idx) * KV_LORA_RANK;
    for (int vt = 0; vt < V_TILES_PER_SG; vt++) {
      const int v_col_start = (sg_n * V_TILES_PER_SG + vt) * FRAG;
      out_row[v_col_start + fn] =
          T(float(V_accum[vt].thread_elements()[0]) * inv_sum);
      out_row[v_col_start + fn + 1] =
  ...
  if (tid < BM) {
    ...
    if (head < num_heads) {
      const uint64_t off =
          (uint64_t)(seq_idx * num_heads + head) * num_partitions + partition_idx;
      max_logits[off] = row_max_smem[tid];
      exp_sums[off] = row_sum_smem[tid];
    }
  }
}

#define instantiate_mla_pr_mma_main(type, kv_lora_rank, qk_rope_head_dim,      \
                                    block_size, partition_size, heads_per_tg,  \
                                    wm, wn)                                    \
  template [[host_name("paged_mla_attention_decode_pr_mma_main_" #type         \
                       "_kvr" #kv_lora_rank "_pe" #qk_rope_head_dim "_bs"      \
                       #block_size "_ps" #partition_size "_hpt" #heads_per_tg  \
                       "_wm" #wm "_wn" #wn)]] [[kernel]] void                  \
  paged_mla_attention_decode_pr_mma_main<type, kv_lora_rank, qk_rope_head_dim, \
                                         block_size, partition_size,           \
                                         heads_per_tg, wm, wn>(                \
      device float *exp_sums       [[buffer(0)]],                              \
      device float *max_logits     [[buffer(1)]],                              \
      device type *tmp_out         [[buffer(2)]],                              \
      device const type *q_combined [[buffer(3)]],                             \
      device const type *latent_cache [[buffer(4)]],                           \
      device const uint32_t *block_tables [[buffer(5)]],                       \
      device const uint32_t *context_lens [[buffer(6)]],                       \
      const constant int &max_num_blocks_per_seq [[buffer(7)]],                \
      const constant int &num_heads_total [[buffer(8)]],                       \
...
// pr_mma target shape: KVR=512, PE=64, ps=128, hpt=32, wm=4, wn=8.
instantiate_mla_pr_mma_main(half, 512, 64, 16, 128, 32, 4, 8);
instantiate_mla_pr_mma_main(bfloat16_t, 512, 64, 16, 128, 32, 4, 8);
instantiate_mla_pr_mma_main(half, 512, 64, 32, 128, 32, 4, 8);
instantiate_mla_pr_mma_main(bfloat16_t, 512, 64, 32, 128, 32, 4, 8);
...
paged_mla_attention_decode_fa(
  ...
  // token index `abs_k`. Clamps physical_block_idx to a valid entry so
  // an OOB lookup returns garbage K (masked to 0 by the softmax mask
  // before contributing to V_accum).
  auto k_base_for = [&](int abs_k) -> const device T * {
    const int pbi_raw = abs_k / BLOCK_SIZE;
    const int pbi =
        pbi_raw < max_num_blocks_per_seq ? pbi_raw : max_num_blocks_per_seq - 1;
...
// ========================================== Instantiations
//
// Main kernel: dtype × block_size × heads_per_tg × partition_size.
//   G=1 → NUM_THREADS=1024 (32 simdgroups, current sdpa_vector layout).
//   G=4 → NUM_THREADS=256  (8 simdgroups, ¼ of the threads but 4× the
//                           per-thread arithmetic — register footprint
//                           per TG stays roughly constant).
// Decode 2pass kernel: dtype × block_size × partition_size — num_heads
// is dynamic via threads_per_threadgroup.y.
// Reduce kernel: dtype × partition_size = 2 × 1 = 2 (PARTITION_SIZE=512
// only), NUM_THREADS=256.
...
// most decode workloads sit at). 16 is in between for medium H.
// PARTITION_SIZE specializations let dispatcher pick a finer split for
// small ctx and a coarser split for long ctx.
...
// ps=512 is already instantiated at line ~1423 for the G-batched single-pass
